...
def mvc_nmf(X, num_epsilon, w_vol, n_iter, err_cut):
    model = MVCNMF(n_components=num_epsilon, regularization=w_vol)
    W, H = model.fit(X, max_iter=n_iter, fit_tolerance=err_cut)
    return W, H